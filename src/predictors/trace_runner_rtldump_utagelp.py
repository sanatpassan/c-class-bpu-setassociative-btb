...
def main():
    bpu = GShareBPU(btb_depth=64, bht_depth=1024)
    debug = False
    lines = load_trace()
    ...
    for i in range(len(lines) - 1):
        ...
        if ci == "Branch":
            ...
            # Sync the architectural global history register state cleanly
            bpu.ghr[0] = (int(actual_taken) << (bpu.MAX_HISTLEN - 1)) | (pred["history"] >> 1)
            bpu.ghr[0] &= (1 << bpu.MAX_HISTLEN) - 1
